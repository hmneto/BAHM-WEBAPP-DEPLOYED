  ...
  mountPointsInTheMap(list, googleMaps) {
    for (let index = 0; index < list.length; index++) {
      ...
      point = googleMaps.createMark(
        googleMaps.getLatLngMaps(element.latitude, element.longitude),
        element.link.indexOf("http") === -1
          ? linkApi+"/imagens/Teste?i=" + element.link
          : element.link
      );
      ...
      point.addListener("click", () => {
        ...
        infoWindow = new google.maps.InfoWindow({
          position: googleMaps.getLatLngMaps(
            element.latitude,
            element.longitude
          ),
        });
        ...
        text.innerHTML = element.nome_ponto;
        div.appendChild(text);
        if (element.tipo_icone !== "KM") text.style.cursor = "pointer";
        text.addEventListener("click", async function () {
          if (element.tipo_icone === "NÃO") return;
          const pa = await post('/GetPaginas',element.pagina)
          pagina = pa
          openView('Page')
        });
